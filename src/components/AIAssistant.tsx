...
// Comprehensive questions database (unchanged from your code)
...
// Speech recognition interface (unchanged)
...
const QuestionCard: React.FC<{
  ...
  return (
    <Card className="p-6 glass bg-gray-300 border border-black space-y-4">
      <div className="flex justify-between items-start">
        <div>
          <div className="text-sm font-light text-black uppercase tracking-wider">
            {question.category} • {question.level} • {question.technology} • Question {index + 1}/{total}
          </div>
          <div className="mt-1 text-xl font-light text-black">{question.question}</div>
        </div>
        <Button
          onClick={() => speakQuestion(index)}
          className="neumorphic bg-black text-white hover:bg-black hover:text-white p-2 rounded-full"
      ...
      {question.tips && (
        <div className="p-3 bg-gray-200 rounded-lg text-sm text-black">
          <span className="font-light">💡 Conseil:</span> {question.tips}
        </div>
      )}

      <div className="space-y-2">
        <label htmlFor="answer" className="block text-sm font-light text-black">
          Votre réponse:
        </label>
        <textarea
          id="answer"
          className="w-full border border-black rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-gray-500 transition"
          rows={5}
          value={currentAnswer}
      ...
      <div className="flex space-x-3">
        <Button
          ...
          className={`neumorphic flex items-center space-x-2 px-4 py-2 text-white hover:text-white font-light ${
        ...
        <Button
          onClick={handleSkip}
          className="neumorphic bg-black text-white hover:bg-gray-800 hover:text-white font-light px-4 py-2"
        >
          Passer
        </Button>
        <Button
          ...
          className={`neumorphic flex-1 px-4 py-2 text-white font-light ${
            !currentAnswer.trim() ? 'bg-gray-500 cursor-not-allowed' : 'bg-black hover:bg-gray-800'
          }`}
        >
...
const ResultsCard: React.FC<{
  ...
  return (
    <Card className="p-6 glass bg-gray-300 border border-black space-y-6">
      <div className="bg-gray-200 p-6 rounded-lg">
        <h2 className="text-2xl font-light text-black mb-2">🎯 Résultats de votre entretien</h2>
        <div className="flex justify-between items-center">
          <div>
            <div className="text-sm text-gray-600">Score total</div>
            <div className="text-2xl font-light text-black">{totalScore}/{maxScore}</div>
          </div>
          <div className="relative w-16 h-16">
            <svg className="w-full h-full" viewBox="0 0 36 36">
              ...
              />
              <div className="absolute inset-0 flex items-center justify-center text-sm font-light text-black">
                {percentage}%
              </div>
            </svg>
          </div>
      ...
      <div className="space-y-4">
        {answers.map((entry, idx) => (
          <Card key={entry.question.id} className="p-4 glass bg-gray-300 border border-black">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-lg font-light text-black">
                  <span className="text-gray-600">Q{idx + 1}:</span> {entry.question.question}
                </p>
              ...
              <div
                className={`px-2 py-1 rounded-full text-xs font-light ${
                  entry.score >= 10 ? 'bg-green-200 text-green-800' : 'bg-amber-200 text-amber-800'
                }`}
              >
                {entry.score.toFixed(1)}/15
              </div>
            </div>
            <div className="mt-2 p-3 bg-gray-200 rounded-lg border border-black">
              <p className="text-sm font-light text-black">Réponse:</p>
              <p className="text-gray-600">{entry.response}</p>
            </div>
            <div
              className={`mt-2 p-3 rounded-lg border border-black ${
                entry.score >= 10 ? 'bg-green-100' : 'bg-amber-100'
              }`}
            >
              <p className="text-sm font-light text-black">Feedback:</p>
              <p className={entry.score >= 10 ? 'text-green-700' : 'text-amber-700'}>{entry.feedback}</p>
              <div className="mt-2">
                <p className="text-xs font-light text-gray-600 mb-1">Critères évalués:</p>
      ...
      <div className="grid grid-cols-2 gap-4">
        <Button
          ...
          className="neumorphic bg-black text-white hover:bg-gray-800 hover:text-white font-light py-3"
        >
          Recommencer
        </Button>
        <Button
      ...
      <Card className="p-4 glass bg-gray-300 border border-black">
        <h4 className="text-lg font-light text-black mb-2">📊 Analyse globale</h4>
        ...
        <div className="mt-3 grid grid-cols-2 gap-2 text-sm">
          <div className="p-2 bg-gray-200 rounded-lg border border-black">
            <div className="text-xs text-gray-600">Temps total</div>
            <div className="font-light text-black">{formatTime(interviewTime)}</div>
          </div>
          <div className="p-2 bg-gray-200 rounded-lg border border-black">
            <div className="text-xs text-gray-600">Moyenne/question</div>
            <div className="font-light text-black">{(totalScore / answers.length || 0).toFixed(1)}/15</div>
          </div>
        </div>
      </Card>
    </Card>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <Card className="p-6 glass bg-gray-300 border border-black">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-2xl font-light text-black">🎙️ JobCoachAI</h2>
              <div className="bg-gray-200 text-black px-3 py-1 rounded-lg text-sm font-light">
              ...
              <>
                <div className="flex flex-wrap gap-4 mb-4">
                  <select
                    ...
                    className="px-3 py-2 bg-gray-200 text-black border border-black rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-500 font-light"
                  >
                  ...
                  <label className="flex items-center space-x-2">
                    ...
                    />
                    <span className="text-sm text-gray-600 font-light">Lecture auto</span>
                  </label>
                </div>
                <QuestionCard
                  question={filteredQuestions[step]}
                ...
                <div className="mt-4">
                  ...
                  <div className="text-xs text-gray-600 text-center mt-1 font-light">
